...
impl EscrowContract {
    ...
    pub fn post_job(
        ...
        add_job_for_client(&e, &client, job_id);
    ...
    pub fn get_client_jobs(e: Env, client: Address) -> Vec<u64> {
        get_client_jobs_storage(&e, &client)
    }
...
fn get_client_jobs_storage(e: &Env, client: &Address) -> Vec<u64> {
    e.storage()
        .persistent()
...
fn add_job_for_client(e: &Env, client: &Address, job_id: u64) {
    let mut jobs = get_client_jobs_storage(e, client);
    jobs.push_back(job_id);
    let key = DataKey::ClientJobs(client.clone());
    e.storage().persistent().set(&key, &jobs);
    e.storage().persistent().extend_ttl(
        &key,
        ACTIVE_JOB_LIFETIME_THRESHOLD,
        ACTIVE_JOB_BUMP_AMOUNT,
