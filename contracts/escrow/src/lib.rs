...
impl EscrowContract {
    ...
    pub fn approve_work(e: Env, client: Address, job_id: u64) {
        ...
        let (fee, payout) = if fee_exempted {
            ...
            let effective_bps = Self::calculate_effective_fee_bps(e.clone(), freelancer.clone()) as i128;
            let amount_tier_bps = calculate_fee_for_amount(&e, job.amount);
            let fee_bps = effective_bps.min(amount_tier_bps);
        ...
        set_job(&e, job_id, &job);
        increment_user_completed_jobs(&e, &freelancer);
        e.storage()
            .persistent()
    ...
    pub fn set_discount_tiers(e: Env, admin: Address, tiers: Vec<DiscountTier>) {
        admin.require_auth();
        let stored_admin = load_admin(&e);
        if admin != stored_admin {
            panic_with_error!(&e, Error::UnauthorizedAdmin);
        }
        e.storage().instance().set(&DataKey::DiscountTiers, &tiers);
        bump_instance_ttl(&e);
    }

    pub fn get_user_completed_jobs(e: Env, user: Address) -> u32 {
        let key = DataKey::UserCompletedJobs(user);
        e.storage().persistent().get(&key).unwrap_or(0)
    }

    pub fn get_discount_tiers(e: Env) -> Vec<DiscountTier> {
        e.storage()
            .instance()
    ...
    pub fn calculate_effective_fee_bps(e: Env, user: Address) -> u32 {
        let base_fee = get_fee_bps_storage(&e) as u32;
        let completed_jobs = Self::get_user_completed_jobs(e.clone(), user);
        let tiers = Self::get_discount_tiers(e.clone());

        let mut max_discount = 0u32;
        for tier in tiers.iter() {
            if completed_jobs >= tier.min_completed_jobs && tier.discount_bps > max_discount {
                max_discount = tier.discount_bps;
            }
        }

        base_fee.saturating_sub(max_discount)
    }
...
fn increment_user_completed_jobs(e: &Env, user: &Address) {
    let key = DataKey::UserCompletedJobs(user.clone());
...
mod test {
    ...
    fn test_discount_tiers_and_high_volume_fee() {
        let (env, client, admin, _user, freelancer, _token) = setup();

        let initial_fee = client.calculate_effective_fee_bps(&freelancer);
        ...
        client.set_discount_tiers(&admin, &tiers);

        assert_eq!(client.get_user_completed_jobs(&freelancer), 0);
        ...
        let discounted_fee = client.calculate_effective_fee_bps(&freelancer);
        assert_eq!(discounted_fee, 200);
    }

    #[test]
    fn test_approve_work_increments_completed_jobs_and_applies_discount() {
        let (env, client, admin, user, freelancer, native_token) = setup();
        ...
        assert_eq!(client.get_user_completed_jobs(&freelancer), 0);
        assert_eq!(client.calculate_effective_fee_bps(&freelancer), 250);

        let amount = 10_000_000i128;
        let job_1 = client.post_job(&user, &amount, &hash(&env), &32u32, &0u64, &native_token);
        client.accept_job(&freelancer, &job_1);
        client.submit_work(&freelancer, &job_1);
        client.approve_work(&user, &job_1);

        assert_eq!(client.get_user_completed_jobs(&freelancer), 1);
        assert_eq!(client.calculate_effective_fee_bps(&freelancer), 150);

        let job_2 = client.post_job(&user, &amount, &hash(&env), &32u32, &0u64, &native_token);
        client.accept_job(&freelancer, &job_2);
        client.submit_work(&freelancer, &job_2);

        let fees_before = client.get_fees(&native_token);
        client.approve_work(&user, &job_2);
        let fees_after = client.get_fees(&native_token);

        let expected_fee = (amount * 150) / 10_000;
        assert_eq!(fees_after - fees_before, expected_fee);
        assert_eq!(client.get_user_completed_jobs(&freelancer), 2);
    }

    #[test]
    #[should_panic(expected = "Error(Contract, #13)")]
    fn test_set_discount_tiers_non_admin_fails() {
        let (env, client, _admin, user, _freelancer, _native_token) = setup();
        let tiers = soroban_sdk::vec![
            &env,
            DiscountTier {
        ...
        client.set_discount_tiers(&user, &tiers);
    }
}
