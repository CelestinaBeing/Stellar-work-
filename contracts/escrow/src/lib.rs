...
impl EscrowContract {
    ...
    pub fn get_job_version(e: Env, job_id: u64) -> u32 {
        let job = get_job_or_panic(&e, job_id);
        job.version
    }

    pub fn migrate_job_version(e: Env, caller: Address, job_id: u64, target_version: u32) -> u32 {
        caller.require_auth();
        require_active_access(&e, &caller);
        let mut job = get_job_or_panic(&e, job_id);

        let admin: Option<Address> = e.storage().instance().get(&DataKey::Admin);
        if Some(caller.clone()) != admin && job.client != caller {
            panic_with_error!(&e, Error::Unauthorized);
        }
        ...
        e.events().publish(
            (Symbol::new(&e, "job_version_migrated"),),
...
mod test {
    ...
    fn test_job_initial_version_is_one() {
        ...
        assert_eq!(client.get_job_version(&job_id), 1);
        let job = client.get_job(&job_id);
    ...
    fn test_migrate_job_version_success() {
        ...
        let new_ver = client.migrate_job_version(&user, &job_id, &2u32);
        assert_eq!(new_ver, 2);
        assert_eq!(client.get_job_version(&job_id), 2);
        let job = client.get_job(&job_id);
        assert_eq!(job.version, 2);

        // Admin migrates version to 3
        let admin_ver = client.migrate_job_version(&admin, &job_id, &3u32);
        assert_eq!(admin_ver, 3);
        assert_eq!(client.get_job_version(&job_id), 3);
    }

    #[test]
    #[should_panic(expected = "Error(Contract, #2)")]
    fn test_migrate_job_version_rejects_unauthorized() {
        let (env, client, _, user, stranger, native_token) = setup();
        let job_id = client.post_job(&user, &1_000_000i128, &hash(&env), &32u32, &0u64, &native_token);
        client.migrate_job_version(&stranger, &job_id, &2u32);
    }
}
